...
export default function Navbar() {
  ...
  style={{
    backgroundColor: menuOpen
      ? '#ffffff' // Açık beyaz, menü açıldığında
      : isTransparent
        ? 'rgba(0, 0, 0, 0.20)' // Sayfada ilk açılışta şeffaf
        : '#ffffff', // Scroll sonrası veya menü kapalıysa beyaz
    borderBottom: menuOpen
      ? '0.5px solid #f0f0f0'
      : isTransparent
        ? '0.5px solid rgba(255,255,255,0.12)' // İlk açılışta şeffaf kenar
        : '0.5px solid #ebebeb', // Diğer durumlarda beyaz kenar
    boxShadow: !isTransparent && !menuOpen
      ? '0 1px 12px rgba(0,0,0,0.06)' // Sayfa kaydırıldığında hafif gölge
      : 'none',
  }}
>
<nav className="container mx-auto px-5 sm:px-8">
  <div className="flex items-center justify-between h-[100px] sm:h-[76px]">
    {/* Logo */}
    <Link href="/" className="hover:opacity-70 transition-opacity duration-300 flex-shrink-0">
      <Image
        src="/uploads/images/silvre-logo-black.png"
        alt="SILVRE"
        width={160}
        height={46}
        className={`h-9 sm:h-11 w-auto transition-all duration-300 ${isTransparent ? 'brightness-0 invert' : ''}`}
        priority
      />
    </Link>
    ...
    >
      ...
      {user ? (
        <div className="relative">
          ...
          {accountOpen && (
            <div className="absolute right-0 mt-3 w-52 bg-white border border-gray-100 rounded shadow-xl py-2 z-50">
              ...
              {/* Menü seçenekleri */}
              <Link href="/orders" className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                Siparişlerim
              </Link>
              <Link href="/addresses" className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                Adreslerim
              </Link>
              <Link href="/profile" className="block px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                Profil
              </Link>
              {user.role === 'admin' && (
                <Link href="/admin" className="block px-4 py-2.5 text-sm text-blue-600 hover:bg-blue-50 transition-colors">
                  Admin Panel
                </Link>
              )}
        ...
        <Link href="/login" className="p-1.5 hover:opacity-50 transition-opacity" aria-label="Giriş Yap">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
            <circle cx="12" cy="8" r="4" />
            <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" />
          </svg>
      ...
      >
        <div className="flex flex-col justify-between h-full px-8 pt-[68px] sm:pt-[76px]">
          ...
          {/* Kategoriler */}
          <div className="flex-1 flex flex-col justify-center px-0 sm:px-16">
